...
fit_onto_forced_response=function(x,g,f.df,h.df,incomplete.years=T,force.h.mean=NULL){
#fit_onto_forced_response=function(x,g,f.df,h.df,incomplete.years=F,force.h.mean=1){
  g=g-mean(g)
  mx=matrix(x,nrow=365)
  if (!incomplete.years) { for (i in 1:ncol(mx)) { if (any(is.na(mx[,i]))) mx[,i]=mx[,i]*NA }}
  fit=t(apply(mx,1,function(x) lm(x~g)$coeff))
  if (!is.null(force.h.mean)) fit[,2]=fit[,2]*force.h.mean/mean(fit[,2])
