...
method.color=function(m){
  out=1
  if (substr(m,1,6)=="calend") out="darkmagenta"
  if (substr(m,1,6)=="annmax") out=2
  if (substr(m,1,6)=="annmin") out=4
  if (substr(m,1,4)=="locm")   out=3
  out
}

method.color2=function(m){
  out=1
  if (substr(m,1,6)=="calend") out="lavenderblush"
  if (substr(m,1,6)=="annmax") out="antiquewhite"
  if (substr(m,1,6)=="annmin") out="aliceblue"
  if (substr(m,1,4)=="locm")   out="palegreen"
  out
}
...
method.name=function(m){
  ...
  if (m=="calend2") out="Calend-SN"
  if (m=="annmax") out="Annual-maxima"
  if (m=="annmin") out="Annual-minima"
  if (m=="annmax1") out="Annmax-GEVcon"
  if (m=="annmin1") out="Annmin-GEVcon"
  if (m=="annmax2") out="Annmax-Gumbel"
  if (m=="annmin2") out="Annmin-Gumbel"
  if (m=="annmax3") out="Annmax-Normal"
  if (m=="annmin3") out="Annmin-Normal"
  if (m=="locmax") out="Local-maxima"
  if (m=="locmin") out="Local-minima"
  if (m=="locmax15") out="Locmax-15d"
  if (m=="locmax30") out="Locmax-30d"
  out
}

method.sample=function(m,w,id1,id2){
  out="SAMPLE"
  if (substr(m,1,6)=="calend") out=paste("on",idays2leg(id1,id2))
  if (substr(m,1,6)=="annmax") out=paste0(w,"-day annual max")
  if (substr(m,1,6)=="annmin") out=paste0(w,"-day annual min")
  if (substr(m,1,6)=="locmax") out=paste0(w,"-day local max")
  if (substr(m,1,6)=="locmin") out=paste0(w,"-day local min")
  out
}
...
var.shortname=function(v){
  out="var"
  if (v %in% c("T","TM","TX","TN")) out="T"
  if (v %in% "GSAT") out="GSAT"
  if (v %in% c("GSAT-ano","GSAT-anoref")) out="GSAT anomaly"
  out
}
...
var.unit=function(v){
  out="varunit"
  if (v %in% c("T","TM","TX","TN","GSAT","GSAT-ano","GSAT-anoref")) out="°C"
  out
}
...
# Calendar axis
my.calendar.axis=function(side=1,mlab=MTHS,dlab=c(1,15),lab=T,lty=3,lwd=0.5,tcl=0.5,ny=3,xlim=NULL){
  d0=makedates(y=1:ny,avg="day",caltype="noleap")
  if (!is.null(xlim)) d0=d0[xlim[1]:xlim[2],]
...
perc2prob=function(lev,percent=F){
  lev=1-lev/100
  if (percent) lev=lev*100
  out=c(); for (l in lev){
...
perc2rp=function(lev)
  round(1/(1-lev/100))

# Time window in titles/legends
idays2leg=function(id1,id2,short=T){
  dd=makedates(y=1:2,avg="day",caltype="noleap")[c(id1,id2),]
...
myfig.detrend=function(s=NULL,var="TM",loc="France",set="mf",
                       y=2022,f.df=NA,g.df=NA,h.df=NA,
                       title=c("","","","")){
  # Data
  if (is.null(s)) s=get.scan.1d(var,"calend",loc,set)
if (s$set %in% c("mf","era5")) s$cmip.period=1850:2100
  var=s$var; meth=s$method; loc=s$location; set=s$set
  if (is.na(f.df)) f.df=s$params$ndf.f
  if (is.na(g.df)) g.df=s$params$ndf.g
  if (is.na(h.df)) h.df=s$params$ndf.h
  # Periods data vs. cmip
  iy=which(s$cmip.period %in% s$period)
  # Multi-model mean
  mmm=s$cmip.mmm
  # Forced response + centered over data period
  Enat=ebm_response(RF,EBMs,s$cmip.period)$be
  dum=forced_response_by_gam_fit(mmm,Enat,g.df)
  ...
  rpanel=c(3,3,3,4,5,6,6)
  if (!is.null(sf_tsx) | !is.null(sf_tsn)) rpanel=c(3,4,5,6,8,8,8)
  layout(cbind(c(1,1,2,2,7,7,7),rpanel),height=c(1,0.3,0.3,0.5,0.4,0.4,0.4))
  #  - annual cycle (1)
  ...
  matplot(s$cmip.period,cbind(mmm,fr),type="l",lty=1,lwd=c(1,2),col=c(1,4),
          axes=F,frame.plot=T,xlim=xlim,ylim=ylim,
  ...
  #  - nat (5)
  par(mar=c(0,3,0,3))
  plot(s$cmip.period,nat,type="l",lwd=2,col=3,axes=F,frame.plot=T,
  ...
  #  - ant (6)
  par(mar=c(3,3,0,3))
  plot(s$cmip.period,ant,type="l",lwd=2,col=2,axes=F,frame.plot=T,
  ...
  for (i in c(1,3)) my.calendar.axis(i,MTHS1,lab=(i==1))
  axis(2,seq(-10,10,1)); axis(2,seq(-10,10,0.05),lab=F,tcl=0.25)
  ...
  #  - annmax/annmin scaling (8, if available)
  if (!is.null(sf_tsx) | !is.null(sf_tsn)){
...
myfig.meth=function(s=NULL,var="TM",meth="calend",loc="France",set="mf",
                    ...
                    title=c("",""),timeseries=T,histogram=T,tinypdf=F,layout=T){
  # Data
  ...
  # Panel plot
  if (timeseries & histogram & layout) layout(matrix(1:2,1,2),widths=c(1,1))
  coly="darkmagenta"
  colm=method.color(meth)
  # Time series plot
  if (timeseries){
    par(mar=c(3,3,1.5,0.1),mgp=c(1.6,0.2,0),tcl=0.5,las=1)
    # expand xlim for barcode
    xlim=range(s$period); xlim=xlim+c(0,0.1)*(xlim[2]-xlim[1])
    xbc=c(max(s$period)+0.02*(xlim[2]-xlim[1]),xlim[2])
    # further expand for tinypdf
    if (tinypdf) xlim=xlim+c(0,0.2)*(xlim[2]-xlim[1])
    # expand ylim for text
    ylim=range(c(dum,dum1),na.rm=T); ylim=ylim+c(-0.1,0.25)*(ylim[2]-ylim[1])
    # empty plot
    plot0(frame.plot=T,xlim=xlim,ylim=ylim,main="",xlab="Years",ylab=var.name(var))
    # time series
    matplot(s$period,cbind(dum,dum1),type="l",lty=1,lwd=c(1,2),col=c("gray",1),add=T)
    # factual/cf worlds
    abline(h=c(dumtr0,dumtr1),lwd=c(2,3),col=c("gray",colm))
    # trend
    lines(s$period,dumtr,lty=5,lwd=1.5,col=colm)
    # value of interest
    #abline(v=y,col=coly); abline(h=dumy,col=coly)
    segments(y,-999,y,dumy,col=coly,lwd=1.5); segments(y,dumy,9999,dumy,col=coly,lwd=1.5)
  ...
  #  text(y,ylim[1],"Event",srt=90,adj=c(0,-0.5),font=2,col=coly)
    ...
    # barcode
    for (i in 1:length(dum1)) segments(xbc[1],dum1[i],xbc[2],dum1[i])
    # tinypdf if requested
    if (tinypdf){
      yy=seq(ylim[1],ylim[2],le=100)
      xx1=xlim[2]-pdf1(yy)*0.8*(xlim[2]-xbc[2])/max(pdf1(yy))
      xx0=xlim[2]-pdf0(yy)*0.8*(xlim[2]-xbc[2])/max(pdf0(yy))
      lines(xx1,yy,lwd=3,col=colm)
      lines(xx0,yy,lwd=2,col="gray")
      mtext(distrib.name(s$params$distrib),side=1,line=0.3,col=colm,at=xlim[2],adj=1,font=2,cex=0.8)
    }
    # Axes, title and legends
    for (i in c(1,3)) axis(i,at=seq(1800,2025,10),lab=(i==1))
    for (i in c(1,3)) axis(i,at=seq(1800,2025,2),lab=F,tcl=0.25)
    for (i in c(2,4)) axis(i,at=seq(-100,100,1),lab=(i==2))
    for (i in c(2,4)) axis(i,at=seq(-100,100,0.2),lab=F,tcl=0.25)
    legend("topleft",bty="n",inset=0.01,
           leg=c(paste(var.shortname(var),c("detr.","raw")),
                 #paste(y,"mean"),paste(s$yref,"mean"),
                 "factual","counter-f.",
                 "trend",paste0("(",round(dumtr1-dumtr0,2)," ",var.unit(var),")")),
           lty=c(1,1,1,1,5,0),lwd=c(1.5,1,3,2,1.5,0),col=c(1,"gray",colm,"gray",colm,0),ncol=3)
    title(paste(title[1],var,loc,method.sample(meth,s$windows[iw],id1,id2)),adj=0)
    if (!tinypdf | histogram) title(method.name(meth),adj=1,col.main=colm)
    if (tinypdf & !histogram) title(paste("p1 =",prob2leg(1-p1),"| p0 =",prob2leg(1-p0)),adj=1,col.main=colm)
  }
  # Histogram plot
  if (histogram){
    ...
    segments(dumtr0,0,dumtr0,pdf0(dumtr0),lwd=2,col="gray")
    plot(pdf1,xlim[1],xlim[2],ylim=ylim,lwd=3,col=colm,add=T)
    segments(dumtr1,0,dumtr1,pdf1(dumtr1),lwd=3,col=colm)
  ...
  #  leg.tr=paste("Estimated trend =",round(dumtr1-dumtr0,2),"°C")
  ...
  #  legend("topright",paste("p1 =",prob2leg(1-pp1),"; p0 =",prob2leg(1-pp0),"; pr=",round(ppr,2)),bty="n")
    leg.ks=round(mykstest(dum1,s$params$distrib,sy$fit1[id,iw,]),2)
    title(paste("p1 =",prob2leg(1-p1),"| p0 =",prob2leg(1-p0),"| pv =",leg.ks),adj=1,col.main=colm)
    legend("topleft",bty="n",inset=0.01,
           #leg=c(paste(y,"climate"),paste(s$yref,"climate")),
           leg=c("factual","counter-f."),
           lty=1,lwd=c(3,2),col=c(colm,"gray"))
  }
}
...
# Warning: s must be a calendar scan!
# MF threshold: above 23.4 for HW and below -2 for CS
#---------------------------------------------------------------------------------
myfig.eve=function(s=NULL,var="TM",loc="France",set="mf",
                   y=2022,xlim=c(1,365*length(y)),id=168,iw=5,
                   title="",rtitle=T,threshold=NULL,exceedance="above",
                   force.anomaly=F,margins=T,show.event=T){
  # Get data of years y-1, y (possibly several) and y+1 (edge effects)
  if (is.null(s)) s=get.scan.1d(var,"calend",loc,set)
  var=s$var; meth=s$method; loc=s$location; set=s$set
  iy=which(s$period %in% y); ny=length(iy)+2; nd=ny*365
  dum=c(); for (i in c(min(iy)-1,iy,max(iy)+1)) dum=c(dum,s$data[,1,i])
  # Get normals of corresp years
#  dumtr1=c(); for (i in c(iy-1,iy,iy+1)) dumtr1=c(dumtr1,s$trend[,1,i])
  mu=sd=c()
  for (yy in c(min(y)-1,y,max(y)+1)){
    mu=c(mu,s$scan[[paste0("Y",yy)]]$fit1[,1,1])
    sd=c(sd,s$scan[[paste0("Y",yy)]]$fit1[,1,2])
  }
  ...
  # Days to plot
  idays=365+(xlim[1]:xlim[2])
  if (show.event) idays=min(id1-30,365+xlim[1]):max(id2+30,365+xlim[2])
  # Data to plot
  dum=dum[idays]; mu=mu[idays]; sd=sd[idays]; mu0=mu0[idays]
  if (force.anomaly) {mu=mu*0; mu0=mu0*NA}
  # Plot
  par(mar=c(3,3,1.5,0.5),mgp=c(1.5,0.1,0),las=1,tcl=0.5)
  if(!margins) par(xaxs="i")
  colm=method.color(meth); colm2=method.color2(meth)
  ...
  # Shading
  if (show.event) polygon(c(id1-0.5,id2+0.5,id2+0.5,id1-0.5),rep(c(-999,999),each=2),col=colm2,border=NA)
  ...
  polygon(c(idays,rev(idays)),c(mu-sd,rev(mu-2*sd)),col="gray90",border=NA)
  if (show.event) abline(v=c(id1-0.5,id2+0.5),col=colm)
  # Threshold exceedance
  if (!is.null(threshold)){
    if (exceedance=="above") cols=c(method.color("annmax"),method.color2("annmax"))
    if (exceedance=="below") cols=c(method.color("annmin"),method.color2("annmin"))
    # add line
    abline(h=threshold,col=cols[1])
    text(max(idays),threshold,paste(threshold,var.unit(var)),font=2,col=cols[1],adj=c(1,-0.5))
    # increase resolution in for (later) plot
    xs=approx(idays,n=length(idays)*100)$y
    ys=approx(dum,n=length(dum)*100)$y
    if (exceedance=="above") ys[ys<threshold]=NA
    if (exceedance=="below") ys[ys>threshold]=NA
  }
  # Axis here because of vertical segments
  for (i in c(1,3)) my.calendar.axis(i,lab=(i==1),ny=ny)
  # Lines
  if (force.anomaly) abline(h=0,lty=5)
  matplot(idays,cbind(mu,mu0,dum),add=T,type="l",lty=c(1,5,1),lwd=c(3,2,1.5),col=c(colm,colm,1))
  if (!is.null(threshold)) lines(xs,ys,lwd=3,col=cols[1])
...
#    for (i in c(2,4)) axis(i,at=seq(-100,100,0.2),lab=F,tcl=0.25)
  axis(2); axis(4,lab=F)
  ytxt=y; if (length(y)>1) ytxt=paste(range(y),collapse="-")
  if (!force.anomaly)
    legend("topleft",bty="n",inset=0.01,ncol=2,
           leg=c(paste(ytxt,"values"),paste(s$yref,"normals"),paste(ytxt,"normals"),"(+/- 1 and 2 sd)"),
           lty=c(1,5,1,0),lwd=c(1,1.5,3,0),col=c(1,colm,colm,0))
  if (force.anomaly)
    legend("topleft",bg="white",box.lty=0,inset=0.02,ncol=1,
           leg=c("values","+/- 1 sd","+/- 2 sd"),
           lty=c(1,0,0),lwd=c(1,0,0),pch=c(0,22,22),pt.cex=c(0,2,2),pt.bg=c(1,"gray80","gray90"))
  if (!is.null(title)) title(paste(title,var,loc,ytxt),adj=0)
  if (rtitle & show.event) title(paste("Event =",idays2leg(mymod(id1,365),mymod(id2,365))),adj=1,col.main=colm)
}
...
myfig.1d=function(s=NULL,var="TM",meth="calend",loc="France",set="mf",
                  y=2003,xlim=c(1,365*length(y)),stat="p1",reverse.prob=F,
                  lev=BRKS1,col=COLSM,title="",rtitle=T,image=T,colorbar=T,double.cb=T,
                  show.events=1,min.value=NULL,min.win=NULL,max.win=NULL){
  # Get data
  if (is.null(s)) s=get.scan.1d(var,meth,loc,set)
  var=s$var; meth=s$method; loc=s$location; set=s$set
  mat=c(); for (yy in y)
    mat=rbind(mat,as.matrix(s$scan[[paste0("Y",yy)]][[paste(stat)]]))
  if (stat %in% c("p1","p0")){
    if (reverse.prob) mat=1-mat
    mat=100*mat
  }
  if (xlim[1]>1) mat[1:(xlim[1]-1),]=NA
  if (xlim[2]<(365*length(y))) mat[(xlim[2]+1):(365*length(y)),]=NA
  ...
  if (colorbar){
    ...
    levleg=lev[2:(length(lev)-1)]; levleg1=round(1/levleg); levleg2=levleg; tit0=""
    if (stat %in% c("p1","p0")) {
      levleg1=perc2rp(levleg); levleg2=perc2prob(levleg)
      levleg0=perc2prob(levleg,percent=T) ; tit0="(%)"
    }
    if (!double.cb) mycolorbar(col,levleg0,horiz=F,width=0.2,cex=1,title=tit0)
    if (double.cb) mycolorbar2(col,levleg1,levleg2,horiz=F,width=0.2,cex=1,titles=c(paste0("1/",stat),stat))
  }
  # Plot 2: image
  if (image){
    par(mar=c(3,3,1.5,0.5),mgp=c(1.5,0.1,0),las=1,tcl=0.5)
    image(1:nrow(mat),1:ncol(mat),mat,axes=F,frame.plot=T,main="",breaks=lev,col=col,
          xlim=xlim,xlab="Central day",ylab="Duration in days")
    # Axes
    for (i in c(1,3)) my.calendar.axis(i,lab=(i==1),ny=length(y))
    ...
    if (!is.null(title)){
      ytxt=y; if (length(y)>1) ytxt=paste(range(y),collapse="-")
      title(paste(title,stat,var,loc,ytxt),adj=0)
    }
    if (rtitle) title(method.name(meth),adj=1,col.main=method.color(meth))
...
#      if (neve>0) points(eve$id[1:neve],eve$iw[1:neve],pch="X",font=2,cex=seq(1.5,0.5,le=10))
      if (neve>0) text(eve$id[1:neve],eve$iw[1:neve],1:neve,font=2,cex=1.5)
...
myfig.chronology=function(e=NULL,var='TM',meth='calend',loc='France',set='mf',
                          ...
                          xlim=c(1,365),ylim=NULL,show.events=0,
                          image=T,colorbar=T,output=F){
  # Get events
  ...
  yy=ylim; if (is.null(yy)){
    yy=range(e$scan.years)+c(-2,2)
    if (xlim[2]<xlim[1]) yy[1]=yy[1]+1
  }
  ...
  if (colorbar){
    par(mar=c(3,0,1.5,1))
    levleg=lev[2:(length(lev)-1)]; levleg1=round(1/levleg); levleg2=levleg
    if (stat %in% c("p1","p0")) {  levleg1=perc2rp(levleg); levleg2=perc2prob(levleg) }
    mycolorbar2(col,levleg1,levleg2,horiz=F,width=0.2,cex=1,titles=c(paste0("1/",stat),stat))
  }
  # Plot 2: image
  if (image){
...
#---------------------------------------------------------------------------------
# Plots the history of extreme events defined as thresholds exceedances
# JJA: xlim=c(152,243)
# DJF: xlim=c(335,59) --> D year y-1 and JF year y
# July to June: xlim=c(182,181)
#---------------------------------------------------------------------------------
myfig.ts.chronology=function(s=NULL,var='TM',loc='France',set='mf',
                             threshold=23.4,min.win=3,exceedance="above",
                             title="",yaxis=T,
                             xlim=c(1,365),xres=10,
                             ylim=NULL,yratio=0.5){
  # Data
  if (is.null(s)) s=get.scan.1d(var,"calend",loc,set)
  var=s$var; meth=s$method; loc=s$location; set=s$set
  # Limits of the plot
  xx=xlim; if (xlim[2]<xlim[1]) xx[2]=xx[2]+365; xs=xx[1]:xx[2]
  yy=ylim; if (is.null(yy)){
    yy=range(s$scan.years)+c(-2,2) ; if (xlim[2]<xlim[1]) yy[1]=yy[1]+1
    if (exceedance=="above") yy[2]=yy[2]+3*yratio
    if (exceedance=="below") yy[1]=yy[1]-3*yratio
  }
  # Increase resolution in x-axis for plot
  myapprox=function(x,n){
    out=rep(NA,n)
    if (sum(!is.na(x))>1) out=approx(x,n=n)$y
    out
  }
  xs=myapprox(xs,xres*(length(xs)-1))
  # Value to plot
  ts=s$data[,1,]
  # Adjust ts to xlim
  if (xlim[2]>xlim[1]) ts=ts[xlim[1]:xlim[2],]
  if (xlim[2]<xlim[1]) {ts=rbind(ts[xlim[1]:365,-ncol(ts)],ts[1:xlim[2],-1]); ts=cbind(rep(NA,nrow(ts)),ts) }
  # Increase resolution in ts
  ts=apply(ts,2,myapprox,xres*(nrow(ts)-1))
  # Generate threshold exceedances
  if (exceedance=="above") ts[which(ts<threshold)]=NA
  if (exceedance=="below") ts[which(ts>threshold)]=NA
  # Minimum duration criterion
  mymindur=function(x,n){
    y=as.numeric(!is.na(x))
    if (any(y==1)) y=mypersist(y,n,0)
    y[y==0]=NA
    x*y
  }
  if (min.win>1) ts=apply(ts,2,mymindur,n=min.win*xres)
  # Resize
  ts=(ts-threshold)*yratio
  # Plot
  par(mar=c(3,3-2.5*(!yaxis),1.5,0.5),mgp=c(1.5,0.1,0),las=1,tcl=0.5)
  plot(0,type="n",xaxs="i",yaxs="i",xlim=xx,ylim=yy,
      axes=F,frame.plot=T,main="",xlab="Calendar days",ylab="")
  abline(h=s$scan.years,col="gray")
  # Plot
  if (exceedance=="above") cols=c(method.color("annmax"),method.color2("annmax"))
  if (exceedance=="below") cols=c(method.color("annmin"),method.color2("annmin"))
  for (y in s$period) polygon(c(xs,rev(xs)),c(y+ts[,which(s$period==y)],rep(y,length(xs))),col=cols[2],border=NA)
  for (y in s$period) lines(xs,ts[,which(s$period==y)]+y,lwd=2,col=cols[1])
  # Axes
  for (i in c(1,3)) my.calendar.axis(i,lab=(i==1))
  ylab=seq(1950,2020,5); abline(h=ylab,lty=3,col='gray')
  axis(2,at=ylab,lab=yaxis & ylab); axis(4,at=ylab,lab=F)
  title(ylab="Years",mgp=c(2,0,0))
  # Titles
  if (!is.null(title)) title(paste(title,var,loc),adj=0)
  title(paste(exceedance,threshold,var.unit(var)),adj=1,col.main=cols[1])
}
  ...
  if (image){
    ...
    ltit=""; if (!is.null(title)){ ltit=title ; if (ltitle){
      ...
      if (!is.null(e$selparams$id) & !is.null(e$selparams$iw))
        dts=paste(dts,idays2leg(e$events$day1[1],e$events$day2[2]))
...
myfig.2d.ggplot=function(e,stat="p1",lev=BRKSM,col=COLSM,title="",ltitle="default",rtitle="default",
                         image=T,colorbar=T,reverse.prob=F,crs=crs.robinson,col.plot=0){
  # Generate image
  if (image){
    # Get map shapefiles
  ...
  if (colorbar){
    levleg=lev[2:(length(lev)-1)]; levleg1=round(1/levleg); levleg2=levleg
    if (stat %in% c("p1","p0")) {  levleg1=perc2rp(levleg); levleg2=perc2prob(levleg) }
    if (!image) mycolorbar2(col,levleg1,levleg2,horiz=F,width=0.2,cex=1,titles=c(paste0("1/",stat),stat))
    if (image){
      plot1=function(){
        par(mar=c(0.5,0,1.5,1))
        mycolorbar2(col,levleg1,levleg2,horiz=F,width=0.2,cex=1,titles=c(paste0("1/",stat),stat))
  ...
  if (image){
    ...
    plot2=ggplot() +
      ...
      theme(plot.title=element_text(hjust=0,size=15,face="bold"),
            ...
            plot.background=element_rect(fill=col.plot),
            plot.margin=margin(0.1, 0.1, -0.3, -0.4, "cm"))
...
hist.all.stat.1d=function(var="TM",meth="calend",loc="France",set="mf",stat="p1",
  ...
  if (meth %in% c("annmax","annmax3","annmin")) ss=apply(ss,c(2,3),max,na.rm=T)
  # Returns a vector
  out=c(ss)
