...
#---------------------------------------------------------------------------------
events2tab.anom=function(e,n=NULL,stat.max="p1",loc=F,locname=F,latex=T,p1=F){
...
#    sel$dleg[i]=dates2leg(cday,cday,short=T)
    sel$dleg[i]=dates2leg(sel$date1[i],sel$date2[i],short=T)
    ano=get.anom.eve(var=e$var,loc=si$loc,set=e$set,y=si$y,id=mymod(si$id,365),iw=si$iw)
    sel$val[i]=round(ano$value,2)
    sel$ano[i]=signif(ano$anonorm,2)
  }
  sel$pp1=signif(100*(1-sel$p1),2)
  # Output matrix
  mat=cbind(sel$n)
  if (loc)     mat=cbind(mat,sel$loc)
  if (locname) mat=cbind(mat,wraf.name(sel$loc))
  mat=cbind(mat,sel$y,sel$dleg,sel$win,sel$val,sel$ano)
  if (p1) mat=cbind(mat,sel$pp1)
  # Matrix 2 latex table
  if (latex) mat=matrix(paste(apply(mat,1,paste,collapse=" & "),"\\\\"),ncol=1)
  # Output
  mat
}
...
# Converts list of events into a latex table
#---------------------------------------------------------------------------------
events2tab.stat=function(e,n=NULL,stat.max="p1",loc=F,locname=F,confint=F,nboot=1000,level=0.9,anom=F,latex=T){
  # Selection of n max events
  if (is.null(n)) sel=e$events
  if (! is.null(n)){
    n=min(n,nrow(e$events))
  ...
  # Calendar anomalies if requested
  if (anom){
    sel$val=sel$ano=NA
    for (i in 1:nrow(sel)){ si=sel[i,]
    ai=get.anom.eve(var=e$var,loc=si$loc,set=e$set,y=si$y,id=mymod(si$id,365),iw=si$iw)
    sel$val[i]=round(ai$value,1) ; sel$ano[i]=round(ai$anonorm,1)
    }
  }
  # Output matrix
  mat=cbind(sel$n)
  if (loc) mat=cbind(mat,sel$loc)
  if (locname) mat=cbind(mat,wraf.name(sel$loc))
  mat=cbind(mat,sel$y,sel$dleg)
  if (anom) mat=cbind(mat,sel$val,sel$ano) 
  mat=cbind(mat,sel$pp1,sel$pp0,sel$ppr)
  # Matrix 2 latex table
  if (latex) mat=matrix(paste(apply(mat,1,paste,collapse=" & "),"\\\\"),ncol=1)
  # Output
  mat
