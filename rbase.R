...
mypersist=function(x,n,replace=NA){
  ...
  if (length(na)>0){ for (i in 1:length(na)) x[(chg[na[i]]+1):chg[na[i]+1]]=replace }
  x
}
...
# Color bar
mycolorbar=function(cols,levs,horiz=TRUE,srt=0,cex=2,width=1,main="",title="",frame.plot=F){
  ...
  if (horiz){
    ...
    text(xlim[1],ycb[1],title,pos=c(3,1),cex=cex*1.1,font=2)
  }
  if (!horiz){
    ...
    text(ycb[2],xlim[2],title,adj=c(-0.3,0.3),cex=cex*1.1,font=2)
  }
}

# Color bar with double labels
mycolorbar2=function(cols,levs1,levs2,horiz=TRUE,srt=0,cex=2,width=1,main="",titles=c("",""),frame.plot=F){
  nl=length(levs1); xlim=c(0,nl+1); ylim=c(0,1)
  if (horiz){
    xcb=seq(xlim[1],xlim[2],le=nl+2);ycb=mean(ylim)+c(-width/2,width/2)
    plot0(xlim=xlim,ylim=ylim,frame.plot=frame.plot)
    for (i in 1:(nl+1)) polygon(xcb[c(i,i+1,i+1,i)],ycb[c(1,1,2,2)],col=cols[i])
    text(xcb[-c(1,length(xcb))],ycb[2],levs1,pos=3,cex=cex,srt=srt)
    text(xcb[-c(1,length(xcb))],ycb[1],levs2,pos=1,cex=cex,srt=srt)
    text(mean(xlim),ylim[1],main,pos=3,cex=cex*1.1,font=2)
    text(xlim[1],ycb[2:1],titles,pos=c(3,1),cex=cex*1.1,font=2)
  }
  if (!horiz){
    xcb=seq(xlim[1],xlim[2],le=nl+2);ycb=mean(ylim)+c(-width/2,width/2)
    plot0(xlim=ylim,ylim=xlim,frame.plot=frame.plot)
    for (i in 1:(nl+1)) polygon(ycb[c(1,1,2,2)],xcb[c(i,i+1,i+1,i)],col=cols[i])
    text(ycb[1],xcb[-c(1,length(xcb))],levs1,pos=2,cex=cex,srt=srt)
    text(ycb[2],xcb[-c(1,length(xcb))],levs2,pos=4,cex=cex,srt=srt)
    text(ylim[2],mean(xlim),main,pos=2,cex=cex*1.1,font=2,srt=90)
    text(ycb[1],xlim[2],titles[1],adj=c(1.1,0.3),cex=cex*1.1,font=2)
    text(ycb[2],xlim[2],titles[2],adj=c(-0.3,0.3),cex=cex*1.1,font=2)
  }
}
